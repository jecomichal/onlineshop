...
$sql = "SELECT title,brand,price,date_added,available, featured FROM Product WHERE product_id = '$pid'";
$rs = $db->query($sql);
...
if($rs->num_rows > 0) {
	...
	while($row = $rs->fetch_assoc()) {
		...
		$outPut  .= 		'</td>
										<td>
											<form>
												<input type="hidden" id="addPID" value="'.$pid.'">
												<button type="button" id="addFeatured" class="btn btn-success" onclick="AddFeatured()">+</button>	
											<form/>															
										</td>
									</tr>';
	}
